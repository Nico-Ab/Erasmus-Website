...
\vspace{0.45cm}
{\large Technical guide for local engineering use\par}
\vspace{1.6cm}
...
\begin{itemize}
  ...
  \item shows a role-based left navigation
...
\subsection{Target deployment profile}
At the moment, the cleanest supported deployment is a single-machine internal installation. In practice that means:

\begin{itemize}
  \item one application host
  \item one PostgreSQL database, either local through Docker or provided separately
  \item one private storage directory on the same host for uploaded files
\end{itemize}
...
\subsection{Important deployment notes before starting}
There are two practical details in the current repository that an engineer should know before deployment.

First, the checked-in Docker setup is fine for local development, but it uses an obvious default database password in \code{docker-compose.yml}. That file should not be copied into an internal environment unchanged.

Second, the current \code{npm run start} script binds the Next.js server to \code{127.0.0.1:3000}. That is correct for local use and for temporary tunnel-based sharing from the same machine. For a server that must be reachable by other machines on an internal network, the final hosting layer needs to account for that. In practice this usually means either adjusting the start binding for the deployment target or placing the app behind a reverse proxy on the same host. The repository does not yet contain that outer hosting configuration.

\subsection{Deployment sequence}
For a clean deployment on another internal machine:

\begin{enumerate}
  \item Install Node.js and Docker on the target machine.
  ...
  \item Set \code{APP_URL} to the URL that users should open on that machine or host.
  \item Set \code{STORAGE_LOCAL_ROOT} to a private directory on disk that will be kept between restarts and backups.
  \item Prepare the database.
  \item Install dependencies with \code{npm install}.
  \item Apply committed migrations with \code{npm run db:migrate:deploy}.
  \item If the target is a demo or evaluation system, optionally run \code{npm run seed}.
...
The first option is to reuse the local Docker model from \code{docker-compose.yml}. That is the fastest way to get a working internal instance. If that path is chosen, the default database name, user, and password should be reviewed and changed before the system is treated as a real shared installation.

The second option is to point \code{DATABASE_URL} at an existing PostgreSQL instance that the university already maintains. From the application side, that is enough as long as the target database is reachable and the Prisma migration command can run against it.

\subsection{Storage preparation}
...
The directory configured through \code{STORAGE_LOCAL_ROOT} should:

\begin{itemize}
  \item be writable by the application process
  \item not sit under a public static web directory
  \item remain available after application restarts
  \item be included in backup planning together with the database
\end{itemize}
...
\subsection{What to edit in the environment file}
The minimum changes before a real internal handoff are:

\begin{itemize}
  \item replace \code{AUTH_SECRET}
  \item confirm \code{APP_URL}
  \item confirm \code{DATABASE_URL}
  \item confirm \code{STORAGE_LOCAL_ROOT}
  \item review upload settings such as \code{MAX_UPLOAD_SIZE_MB} and \code{ALLOWED_UPLOAD_EXTENSIONS}
\end{itemize}

If the deployment is intended for Bulgarian-first use, the engineer can also change \code{DEFAULT_LOCALE} from \code{en} to \code{bg}.
...
\subsection{First checks after deployment}
After the application has started, the first checks should be simple:

\begin{enumerate}
  \item open the configured base URL
  \item open \code{/status}
  \item open \code{/api/health}
  \item sign in with a seeded or known account
  \item open a protected dashboard page
  \item if the system will be used for real workflows, upload and download one document through the protected flow
\end{enumerate}

This is enough to confirm that the application server, database, auth layer, and private storage are all working together.

\subsection{Routine update process}
For later updates on the same host, the basic sequence stays simple:

\begin{enumerate}
  \item pull the latest repository state
  \item review any environment changes
  \item run \code{npm install} if dependencies changed
  \item run \code{npm run db:migrate:deploy}
  \item run \code{npm run build}
  \item restart the production application process
\end{enumerate}

If the update includes schema changes, the migration step should not be skipped.

\subsection{What the local engineer will still need to decide}
The repository brings the application itself, but an internal deployment still needs a few local decisions:

\begin{itemize}
  \item whether PostgreSQL is run through the included Docker setup or an existing university database
  \item where the private storage directory will live
  \item how the application process is kept running after restarts
  \item whether the application is only for same-machine use, internal network use, or temporary public sharing
  \item whether a reverse proxy and TLS are required for the target environment
\end{itemize}
